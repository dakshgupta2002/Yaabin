<script>
	...
	export let preview;
	let resRight; 	let resLeft; 	let resCenter;
	onMount( async () => {
		resLeft = await fetchImage();
		resRight = await fetchImage();
		resCenter = await fetchImage();

		document.getElementById('leftImg').src = resLeft.message;	
		document.getElementById('rightImg').src = resRight.message;	
	
		document.getElementById('centerImg').src = resCenter.message;	
		document.getElementById('body').innerHTML = resCenter.message.split('/')[5];
    });
	...
	window.addEventListener('click', (event) => {
		if (event.clientX > window.innerWidth/2 && event.clientY > window.innerHeight/10
		 && event.clientY< window.innerHeight){
			event.preventDefault();
			leftMouseDown=true;
		}
		if (event.clientX < window.innerWidth/3 && event.clientY > window.innerHeight/10
	...
	window.addEventListener('mouseup', (event) => {
		if (event.clientX > window.innerWidth/3 && event.clientX< window.innerWidth/2
		 && event.clientY > window.innerHeight/20 && event.clientY< 4*window.innerHeight/5){
			event.preventDefault();
			if (leftMouseDown){
				leftMouseDown=false;
				left();
...
<style>
	...
	/* .leftTransition{
		animation: leftTransition 5s ease-in-out 0s 1 forwards;

	}
	.rightTransition{
		animation: rightTransition 5s ease-in-out 0s 1 forwards;
	}
